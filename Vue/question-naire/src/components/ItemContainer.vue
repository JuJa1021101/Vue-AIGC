<template>
    <div>
      ...
      <div v-if="parent === 'item'">
        ...
        <span class="next_item button_style" @click="nextQuestion" v-if="state.itemNum < state.questionList.length">
          
        </span>
        <span class="submit_item button_style" @click="submitQuestion" v-else>
...
const { state } = storeToRefs(questionStore)//变成响应式
...
const nextQuestion = () =>{
  ...
  // state.answerList.push(currentNum.value+(state.itemNum-1)*4)
  questionStore.saveAnwerList(currentNum.value)
